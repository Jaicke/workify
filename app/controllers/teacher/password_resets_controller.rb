class Teacher::PasswordResetsController < ApplicationController
  layout 'sign'
  ...
  def create
    @user = Teacher::User.find_by(email: params[:email])

    respond_to do |format|
      if @user.present?
        @valid_email = true
        Teacher::UserMailer.with(user: @user).password_reset_email.deliver_later
