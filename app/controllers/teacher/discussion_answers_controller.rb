class Teacher::DiscussionAnswersController < Teacher::BaseController
  ...
  def create
    ...
    if @discussion_answer.save
      redirect_to teacher_discussion_path(id: @discussion.id, work_id: @work.id), notice: 'Sua resposta foi adicionada com sucesso'
    end
  end
  ...
  def update
    if @discussion_answer.update(update_discussion_answer_params)
      redirect_to teacher_discussion_path(id: @discussion.id, work_id: @work.id), notice: 'Sua resposta foi atualizada com sucesso.'
    else
      render :edit
    end
  end
  ...
  def toggle_like
    ...
    redirect_to teacher_discussion_path(id: @discussion.id, work_id: @work.id)
  end

  def destroy
    @discussion_answer.destroy
    redirect_to teacher_discussion_path(id: @discussion.id, work_id: @work.id), notice: 'Sua resposta foi removida.'
  end
